...
I have finished approximately 30%. This is just a rough guess, derived from my @sec-project-plan.

Currently I am working on setting up a new navigational structure in @sec-chap101. I try to implement this new user interface with the different --- already prepared --- visualizations: line charts for score values, bump charts for rank values, and maps for the country distribution of score values.
...
I decided to develop a dashboard for the dataset on the World Press Freedom Index (`r glossary("WPFI")`) for this practical learning project. The WPFI was developed by [Reporters Without Borders](https://rsf.org/en/index) (`r glossary("RWB")`) (French: Reporters sans frontières) (RSF).
...
I take as a model for my practical project the [Word Happiness Report](https://data.worldhappiness.report/map) (`r glossary("WHR")`) dashboard. I cannot plan the details here but my dashboard (see for the current state of the project plan chapter @sec-chap061) should contain the following main visualizations:
...
I will apply the intermediate results of my code development parallel the [Shiny package RSF](https://github.com/petzi53/rsf) (Reporters sans frontières). After some research I learned that the {**bslib**} package is the most modern package for dashboard development. I looked into
...
I have used for this book the first time the {**renv**} package. For this reason I have changed my standard `sessioninfo::session_info()` to the more succinct version `xfun::session_info()`. This has also the advantage that the full path to my local files on my computer is not revealed anymore. 
But actually I do not know, if I still would need session information, because {**renv**} provides all necessary information and handling to reproduce my project exactly.
